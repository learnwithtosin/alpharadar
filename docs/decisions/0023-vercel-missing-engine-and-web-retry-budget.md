...
## 1d. Deployed anyway — the wasm file still didn't survive Vercel's real packaging, despite passing every local check

Deployed with 1c's fix. The native-engine error stayed gone and the retry classifier held (both real confirmations that the adapter migration itself is sound). But a narrower error appeared:

```
ENOENT: no such file or directory, open '/var/task/apps/web/generated/client/query_compiler_bg.wasm'
```

Right path — matching exactly what 1c's local verification confirmed and copied to — but the file itself missing from the deployed function. The build had not failed (the copy script runs synchronously and throws if its source is missing, so a silent partial failure there was already ruled out), meaning the file existed on Vercel's own build machine but did not survive into `/var/task`. The same category of local-passes/Vercel-differs gap as 1b's `.so.node` issue, this time for a file placed correctly inside the app's own project root (not crossing a package boundary the way the `.so.node` did) — so the earlier theory ("outputFileTracingIncludes can't reach outside the project root") doesn't explain this one; something about Vercel's real packaging specifically doesn't reliably ship this file, for a reason not otherwise observable without Vercel CLI/dashboard access (unavailable from this sandbox — no network path to install it).

**Read the actual bundled server chunk directly, not the pre-bundled source**, to get the real mechanism rather than continuing to guess between candidates. Prisma's compiled loading code, extracted verbatim from `.next/server/chunks/48.js`:
```js
...
```
This confirms the path is genuinely `process.cwd() + "/generated/client"` (not a build-time constant), which is why both the local standalone run and Vercel's own logs independently name the identical app-relative path — strong confirmation the *destination* was never wrong. It also revealed the fallback checks for `schema.prisma` specifically (defaulting to `"generated/client"` regardless if neither candidate has one, so this wasn't itself the failure — but copying it too removes that as a variable rather than leaving it as a loose end).

Given the destination was confirmed correct and the remaining gap is specifically about surviving Vercel's real packaging (unverifiable further from here), applied three independent, complementary hardenings rather than one more guess:

1. **Removed the dependency on package.json's `prebuild`/`predev` lifecycle hooks entirely.** Whether Vercel's configured build command actually triggers pnpm's pre-script behavior is not something confirmable without dashboard access — pnpm 9 runs pre/post scripts by default, but that only applies if Vercel invokes the build *through* a named npm/pnpm script (`pnpm run build`) rather than, say, an overridden literal `next build` command. `copyPrismaWasmRuntime()` (exported from `scripts/copy-prisma-wasm.mjs`) is now called directly from `next.config.ts`'s top-level module code — guaranteed to run every time `next build`/`next dev`/`next start` loads its config, regardless of what shell command got there, since Next.js loading its own config file is not optional. `apps/web/package.json`'s `prebuild`/`predev` now only run `prisma generate` (still needed, still apparently working, left alone).
2. **Also copies `schema.prisma`** into the same destination — the fallback logic above checks for it; even though the "generated/client" default is chosen either way here, this removes it as a variable rather than leaving an unexplained loose end.
3. **Tightened `outputFileTracingIncludes` to name the three exact files, not a wildcard glob** (`./generated/client/query_compiler_bg.wasm` etc., not `./generated/client/*`) — removes any ambiguity about glob-matching behavior that a wildcard leaves open.

None of these can be verified against Vercel's actual packaging from this sandbox — flagged plainly rather than claimed as certain. What *is* re-verified, to the same standard as before: a real `next build`, followed by actually running the real standalone `server.js` with `cwd` set to `apps/web` (matching the Lambda's own `process.cwd()`, confirmed via the ENOENT paths above), confirms all three files physically present at the exact path the bundled code computes, and real requests against `/opportunities` and `/` return `HTTP 200` with genuine live data (a real, current `SCAN 12m ago`, not a stale fixture). This is the strongest local proxy available; whether it finally matches Vercel's own packaging is only knowable from the next real deploy.
...
- §1d's fix (config-load-time copy, exact-file tracing includes) has not yet been deployed. This is now the *third* round where a fix passed every available local check (including, in 1c and 1d, an actual standalone server serving real production data) and Vercel's real packaging still differed — that pattern itself is now the main open risk, not any one specific remaining bug. If §1d's fix still doesn't survive a real deploy, the next step should be getting real Vercel Build Output ground truth (`vercel build` via the CLI) rather than a fourth local-only theory, since this sandbox has no network path to install that CLI and local proxies have now twice failed to predict Vercel's actual behavior.
...
- **Ran a real `next build`, then the actual standalone `server.js`, against the real database** — twice, once for §1c and again for §1d after the config-load-time copy and exact-file tracing changes: `/opportunities` and `/` both returned `HTTP 200` with genuine rendered data (real opportunity rows; a live, current `SCAN Xm ago` from the real `IngestionCheckpoint`, not a stale fixture) — not just a trace-manifest listing or a build that exits 0, but the complete path exercised end-to-end against production data, from a bundle shaped like what Vercel would deploy.
- §1d specifically: read the actual compiled logic out of the real bundled server chunk (`.next/server/chunks/48.js`) to confirm the exact runtime path-resolution mechanism, rather than continuing to infer it from source or documentation — this is what settled that the copy *destination* was already correct and the remaining gap was specifically about surviving Vercel's packaging.
...
- **Not verified, and flagged as such rather than assumed**: whether §1d's fix actually survives Vercel's real deployment packaging. Two prior rounds (1b, 1c) each passed this exact same local verification standard and still failed once deployed — that gap is a property of Vercel's real build pipeline this sandbox cannot observe directly (no network path to install the Vercel CLI for `vercel build` ground truth), not something a fourth local check can resolve on its own.
